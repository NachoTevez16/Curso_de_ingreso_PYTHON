...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):
        ...
        cantidad_camiones = round(toneladas/3500)
        msg =f"Se necesitaran {cantidad_camiones} camiones para transportar {toneladas} kilos"
        alert("",msg)

    def btn_tiempo_llegada_on_click(self):
        kilometros = float(self.txt_kilometros.get())
        cantidad_horas = kilometros/90
        msg =f"Se necesitaran {cantidad_horas} horas para recorrer {kilometros} kilometros"
        alert("",msg)
